<template>
  <div ref="selectRef" v-click-outside:[popperRef]="handleClickOutside" :class="[nsSelect.b(), nsSelect.m(selectSize)]"
    @mouseenter="states.inputHovering = true" @mouseleave="states.inputHovering = false">
    <el-tooltip ref="tooltipRef" :visible="dropdownMenuVisible" :teleported="teleported"
      :popper-class="[nsSelect.e('popper'), popperClass!]" :popper-style="popperStyle" :gpu-acceleration="false"
      :stop-popper-mouse-event="false" :popper-options="popperOptions" :fallback-placements="fallbackPlacements"
      :effect="effect" :placement="placement" pure :transition="`${nsSelect.namespace.value}-zoom-in-top`"
      trigger="click" :persistent="persistent" :append-to="appendTo" :show-arrow="showArrow" :offset="offset"
          ...
          <div ref="selectionRef" :class="[
            ...
            <slot v-if="multiple" name="tag" :data="states.cachedOptions" :delete-tag="deleteTag"
              ...
              <div v-for="item in showTagList" :key="getValueKey(getValue(item))" :class="nsSelect.e('selected-item')">
                <el-tag :closable="!selectDisabled && !getDisabled(item)" :size="collapseTagSize" :type="tagType"
                  :effect="tagEffect" disable-transitions :style="tagStyle" @close="deleteTag($event, item)">
                  <span :class="nsSelect.e('tags-text')">
                    <slot name="label" :index="getIndex(item)" :label="getLabel(item)" :value="getValue(item)">
                      {{ getLabel(item) }}
                    </slot>
                  </span>
                </el-tag>
              ...
              <el-tooltip v-if="collapseTags && modelValue.length > maxCollapseTags" ref="tagTooltipRef"
                ...
                <template #default>
                  <div ref="collapseItemRef" :class="nsSelect.e('selected-item')">
                    <el-tag :closable="false" :size="collapseTagSize" :type="tagType" :effect="tagEffect"
                      :style="collapseTagStyle" disable-transitions>
                      <span :class="nsSelect.e('tags-text')">
                ...
                <template #content>
                  <div ref="tagMenuRef" :class="nsSelect.e('selection')">
                    <div v-for="selected in collapseTagList" :key="getValueKey(getValue(selected))"
                      :class="nsSelect.e('selected-item')">
                      <el-tag class="in-tooltip" :closable="!selectDisabled && !getDisabled(selected)"
                        :size="collapseTagSize" :type="tagType" :effect="tagEffect" disable-transitions
                        @close="deleteTag($event, selected)">
                        <span :class="nsSelect.e('tags-text')">
                          <slot name="label" :index="getIndex(selected)" :label="getLabel(selected)"
            ...
            <div :class="[
              ...
              <input :id="inputId" ref="inputRef" :value="states.inputValue" :style="inputStyle"
                :autocomplete="autocomplete" :tabindex="tabindex" aria-autocomplete="none" aria-haspopup="listbox"
                ...
                @keydown.enter.stop.prevent="onKeyboardSelect" @keydown.esc.stop.prevent="handleEsc"
                @keydown.delete.stop="handleDel" @click.stop="toggleMenu" />
            ...
            <div v-if="shouldShowPlaceholder" :class="[
              ...
              <slot v-if="hasModelValue" name="label" :index="allOptionsValueMap.get(modelValue)?.index ?? -1"
                :label="currentPlaceholder" :value="modelValue">
      ...
      <template #content>
        <el-select-menu :id="contentId" ref="menuRef" :data="filteredOptions"
          :width="popperSize - BORDER_HORIZONTAL_WIDTH" :hovering-index="states.hoveringIndex"
          :scrollbar-always-on="scrollbarAlwaysOn" :aria-label="ariaLabel">
